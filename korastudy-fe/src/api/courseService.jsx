...
const courseService = {
  ...
  completeLesson: async (courseId, lessonId) => {
    try {
      ...
      console.error(
        `Lỗi khi đánh dấu hoàn thành bài học ID=${lessonId}:`,
        error
      );
      throw error;
    }
  ...
  getUserEnrollments: async (userId = null) => {
    try {
      // Backend EnrollmentController exposes /api/v1/enrollments/... endpoints
      const url = userId
  ...
  getCourseEnrollments: async (courseId, page = 0, size = 10) => {
    try {
      ...
      console.error(
        `Lỗi khi lấy danh sách học viên của khóa học ID=${courseId}:`,
        error
      );
      throw error;
    }
